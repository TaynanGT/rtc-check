...
Regras:
- Use `Grep` e `Glob` antes de `Read`. Só leia o trecho necessário, nunca o
  arquivo inteiro por precaução.
- Nunca leia `.env`, `credentials/`, `*.pem`, `*.key`, `id_rsa*` nem diretório de
  dado de cliente.
- Responda em no máximo 15 linhas.

Formato da resposta:
1. Resposta direta em uma frase.
2. Lista de `caminho/do/arquivo:linha` — o que há em cada um, em meia linha.
