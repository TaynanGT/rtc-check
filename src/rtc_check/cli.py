...
def analisar(
    ...
    for caminho in varrer_pasta(pasta, recursivo):
        ...
        emitente = resumo.emitentes.setdefault(
            nota.emitente_cnpj or "(sem CNPJ)",
            ResumoEmitente(cnpj=nota.emitente_cnpj or "(sem CNPJ)", nome=nota.emitente_nome),
        )
        emitente.notas += 1
        emitente.notas_em_escopo += int(nota.em_escopo_agosto)
        emitente.itens += len(nota.itens)

        for achado in avaliar_nota(nota, regras):
